...
● Original set contains 7,824,482 observations and 4 variables - User ID, Product ID, Rating (from 1 to 5), and Time Stamp. It covers 4,201,696 users and 476,001 products. In order to make the set more manageable it has been reduced to a smaller subset.
...
```{r Transform Data}
class(ratings_data$userId)
class(ratings_data$productId)
class(ratings_data$Rating)
class(ratings_data$timestamp)
...
( amazonShort <- UI[rowCounts(UI) > 20, colCounts(UI) > 40] )
...
class(ratings$userId)
class(ratings$productId)
class(ratings$Rating)

amazon <- as(ratings, "realRatingMatrix")
dim(amazon@data)
```
